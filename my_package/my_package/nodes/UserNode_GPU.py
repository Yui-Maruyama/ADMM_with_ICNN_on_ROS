...
class UserNode_GPU(Node):
    def __init__(self):
        ...
        match self.scene:
            case 1:
                self.model = myPICNN(22, 10, 100).to(self.device)
                self.model.load_state_dict(torch.load('../model/model_scene_1.pth'))
                self.model.train()
                self.model = torch.compile(self.model)
            case 2:
                self.model = myPICNN(16, 10, 100).to(self.device)
                self.model.load_state_dict(torch.load('../model/model_scene_2.pth'))
                self.model.train()
                self.model = torch.compile(self.model)
            case 3:
                self.model = myPICNN(30, 10, 100).to(self.device)
        ...
        with open(capacity_info_file, 'r') as file:
            for line in file:
                ...
                self.max_capacities.append(torch.tensor(self.numbers, device=self.device, requires_grad=False))
        self.max_capacity = self.max_capacities[self.scene - 1]

        # 変数の初期化
        self.x = torch.tensor(np.zeros(len(self.max_capacity) * 2), device=self.device, requires_grad=True)
        self.mu = {user_id: torch.tensor([0.0], device=self.device) for user_id in self.neighbors}
        self.s = {user_id: torch.tensor([0.0], device=self.device) for user_id in self.neighbors}
        self.rho = 1.0
        self.x_list = []
        self.mu_list = []
        self.s_list = []
        ...
        self.other_usages = {user_id: torch.tensor([0.0], device=self.device) for user_id in self.neighbors}

        # オプティマイザ
        self.optimizer = optim.Adam([self.x], lr=0.01)

        # 結果記載用ファイルの初期化
        self.result_file_name = f"../result_user_{self.total_users}_GPU/user_{self.user_id}.txt"
    ...
    def optimization_loop(self):
        ...
        while rclpy.ok() and (not is_converged(self.scores, self.step)):
            ...
            x = local_optimize(self.user_id, self.x, self.other_usages, self.mu, self.s, self.rho, self.optimizer, self.model, self.neighbors, self.max_capacity, self.bandwidth, iter_inner = 3, ctx = torch.tensor(1, device=self.device))
            ...
            # スコアの更新
            self.scores, self.x_list, self.mu_list, self.s_list = update_score_GPU(self.model, self.x, self.mu, self.s, self.x_list, self.mu_list, self.s_list, self.scores, self.result_file_name, time.time() - self.start_time, ctx = torch.tensor(1, device=self.device))
